...
final class TweetRepositoryInMemory implements TweetRepository
{
    ...
    public function searchByUserName(TweetUsername $username, TweetLimit $limit): ?array
    {
        return $this->getUserTweets($username)
            ? $this->createRandomTweets($this->getUserTweets($username), $limit->getValue())
            : null;
    }

    private function createRandomTweets(array $userTweets, int $limit): array
    {
        $randomEntries = array_rand($userTweets, min($limit, count($userTweets)));

        return array_map(
            fn($randomEntry) => new Tweet($userTweets[$randomEntry]),
            (array)$randomEntries
        );
    }

    private function getUserTweets(TweetUsername $username): ?array
    {
        return $this->tweets[$username->getValue()] ?? null;
    }
}
